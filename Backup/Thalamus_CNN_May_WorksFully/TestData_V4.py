...
def TestData(net , Directory_Nuclei_Test , Directory_Nuclei_Train , OriginalSeg , subFolders, CropDim , padSize):
...
def TestData2_MultThalamus(net , Directory_Nuclei_Test , Directory_Nuclei_Train , OriginalSeg , subFolders, CropDim , padSize , Directory_Thalamus_Test , Directory_Thalamus_TrainedModel , NucleusName):
...
def TestData2_MultThalamus_PerSlice(net , Directory_Nuclei_Test0 , Directory_Nuclei_Train0 , OriginalSeg , subFolders, CropDim , padSize , Directory_Thalamus_Test , Directory_Thalamus_TrainedModel , NucleusName , SliceNumbers,Manual_Thalamus):
    ...
    for sliceInd in range(L):
        ...
        Directory_Nuclei_Train_Model_cpkt = Directory_Nuclei_Train + 'model/model.cpkt'
        Directory_Nuclei_Test_Results   = Directory_Nuclei_Test  + 'results_MultThalamus/'
        ...
        # PredictedSeg = prediction[0,...,1] > 0.1
        try:
            Thresh = max(filters.threshold_otsu(prediction[0,...,1]),0.2)
        except:
            Thresh = 0.2
    ...
    Directory_Nuclei_TestOutput = Directory_Nuclei_Test0 + 'ResultsMultThalamus/'
    try:
    ...
    nib.save(Prediction3D_nifti,Directory_Nuclei_TestOutput + subFolders + '_' + NucleusName + '.nii.gz')

    Prediction3D_logical_nifti = nib.Nifti1Image(Prediction3D_Mult_logical,Affine)
    Prediction3D_logical_nifti.get_header = Header

    nib.save(Prediction3D_logical_nifti,Directory_Nuclei_TestOutput + subFolders + '_' + NucleusName + '_Logical.nii.gz')

    return Prediction3D_Mult, Prediction3D_Mult_logical
