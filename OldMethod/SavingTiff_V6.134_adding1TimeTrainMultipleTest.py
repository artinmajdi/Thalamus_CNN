...
def input_GPU_Ix():
    ...
    UserEntries['testIx'] = [1,5,10,14,20]
    UserEntries['testIx_Flag'] = 0
    ...
    for input in sys.argv:
        ...
        elif 'testIx' in input:
            UserEntries['testIx_Flag'] = 1
            # if input.split('=')[1][0] == '[':
            #     B = input.split('=')[1].split('[')[1].split(']')[0].split(",")
            #     UserEntries['testIx'] = [int(k) for k in B]
            # else:
            #     UserEntries['testIx'] = [int(input.split('=')[1])]
...
def OneTrain_MultipleTest(UserEntries , Params , subFolders,imFull,mskFull):
    ...
    for sFi in UserEntries['testIx']:

        Dir_Each = mkDir(Params['Dir_EachTraining'] + '/OneTrain_MultipleTest' + '/Test' + '/' + subFolders[sFi])
        for slcIx in range(imFull.shape[2]):
    ...
    for sFi in range(len(subFolders)):
        if sFi not in UserEntries['testIx']:
...
for ind in UserEntries['IxNuclei']: # 1,2,8,9,10,13]: #
    ...
    for ii in UserEntries['enhanced_Index']: # len( Params['A'] ):
        ...
        if UserEntries['testIx_Flag'] == 0:
            normal_Cross_Validation(Params , subFolders , imFull , mskFull)


        elif UserEntries['testIx_Flag'] == 1:
