...
class Unet(object):
    ...
    def predict(self, model_path, x_test, GPU_Num="1"):
        """
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False, write_graph=False, prediction_path = 'prediction', GPU_Num="1"):
        """
...
def error_rate(predictions, labels):
    ...
    """
    # with tf.device('/device:GPU:2'):
    A = 100.0 - ( 100.0 * np.sum(np.argmax(predictions, 3) == np.argmax(labels, 3)) /
